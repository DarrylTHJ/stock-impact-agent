...
- `scripts/verify_chen_drafts.py` is a separate semantic-evidence gate. It batches candidate records with their fixed source quotes and asks Gemini only whether target, direction, reason, market context, and named companies are explicitly supported. Local rules then approve the record, demote it to `market_context`, or reject it. Verified outputs are stored separately in `data/chen_verified_drafts/`; only these can be promoted. A tariff → Transportation & Logistics candidate was correctly rejected, while a Starlink → Telecommunications companies record was accepted after preserving the source-named industry.
