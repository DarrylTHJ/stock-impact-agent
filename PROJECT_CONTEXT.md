...
- A public YouTube caption collector now exists at `scripts/collect_chen_captions.py`. It stores one ignored local JSON file per Chen video in `data/chen_source_captions/`, including video metadata and timestamped original-language caption segments. It records unavailable/error cases in `collection_log.jsonl`, and uses automatic captions only if no manual Chinese/English caption is available. A one-video test successfully saved a manual Chinese transcript with 621 segments.
- The completed collection saved 430 captioned Chen videos; 100 videos without supported captions were logged and excluded.
- `scripts/transform_chen_transcripts.py` sends one full timestamped transcript at a time to Gemini 3.1 Flash-Lite and creates extracted knowledge records in `data/chen_knowledge_records/`. It uses a 45-second delay, checkpoints completed work, and stops on quota/rate-limit errors. Do not start a mass transformation until prompt quality is reviewed on representative videos.
...
- The Chen stages now use clear folders: `chen_source_captions` (raw source), `chen_knowledge_records` (initial extraction), `chen_initial_support_checks` (first semantic check), `chen_recovered_evidence` (additional source evidence), and `chen_final_verified_records` (only records eligible for human approval/promotion). `scripts/verify_chen_drafts.py` performs the support check; `scripts/recover_chen_evidence.py` searches the full transcript after a `needs_more_evidence` result. A tariff → Transportation & Logistics record was correctly rejected after recovery, while a Starlink → Telecommunications companies record was accepted after preserving the source-named industry.
