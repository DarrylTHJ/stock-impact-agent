...
- `scripts/transform_chen_transcripts.py` sends one full timestamped transcript at a time to Gemini 3.1 Flash-Lite and creates candidate local records in `data/chen_candidate_records/`. It uses a 45-second delay, checkpoints completed work, and stops on quota/rate-limit errors. Do not start a mass transformation until prompt quality is reviewed on representative videos.
