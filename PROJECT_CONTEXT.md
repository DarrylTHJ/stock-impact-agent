...
- HLIB currently has 149 source PDFs in `data/hlib_source/`: 28 Market View, 6 Economic Update, and 115 Industry Insight reports. `scripts/extract_hlib_pdfs.py` locally extracts page-preserving JSON into `data/hlib_extracted/`; all 149 reports extracted successfully. `scripts/transform_hlib_reports.py` is prepared for one full-report Gemini call per report and writes initial records to `data/hlib_transformed_initial/`. It preserves page-number evidence, ignores rating/target-price tables unless accompanied by causal analysis, and records source-quote matching warnings for later validation.
